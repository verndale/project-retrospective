...
- Progressive disclosure is one level deep: every `references/*.md` links directly from `SKILL.md`, and no reference links to another reference. The references are leaf nodes by design, and the knowledge graph now proves it — the reference subgraph is carried entirely by `contracts` edges.
