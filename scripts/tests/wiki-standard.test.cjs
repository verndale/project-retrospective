...
const { formatRoute, route } = require("../graph/routing.cjs");
...
test("all closing issues are extracted across same-repo, cross-repo, and URL forms", () => {
  const refs = extractClosingIssues(
    "Closes #9 and #10. Related #99 is not closing.\n~~~md\nFixes hidden/repo#98.\n~~~\nFixes Other/Repo#11. Resolved https://github.com/third/repo/issues/12.",
...
test("issue refresh ignores fenced citations and headings", (t) => {
  ...
  assert.doesNotMatch(fs.readFileSync(file, "utf8"), /issues\/(?:90|91|93)\) — closed/);
});
