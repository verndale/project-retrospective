...
async function init() {
  ...
  buildIndexes();
  buildModel();
  buildRenderer(await waitForContainer($("#graph")));
  buildLegend();
...
// Sigma measures its container at construction and throws "Container has no width" if the
// browser has not laid it out yet — which is the normal state for a background tab, and
// for a first paint that lands after the fetches resolve. Wait a few frames for a real
// box; if one never arrives there is nothing to draw into anyway, so fall through with
// allowInvalidContainer and let Sigma pick the size up on its next resize rather than
// leaving the panel on "loading…" forever.
function waitForContainer(el, timeoutMs = 2000) {
  if (el.getBoundingClientRect().width > 0) return Promise.resolve(true);
  return new Promise((resolve) => {
    const started = Date.now();
    const tick = () => {
      if (el.getBoundingClientRect().width > 0) resolve(true);
...
function buildRenderer(sized = true) {
  state.renderer = new Sigma(state.model, $("#graph"), {
    allowInvalidContainer: !sized,
...
function renderPanel(id) {
  ...
  if (refs.length) {
    ...
    // Older generated graphs remain readable while a consumer rebuilds. These
    // number-only fields cannot safely become guessed repository URLs.
...
// init() is async, so without this a throw anywhere inside it becomes an unhandled
// rejection and the panel sits on "loading…" forever with nothing in the console.
init().catch((error) => {
  const stats = $("#stats");
  if (stats) stats.textContent = `Viewer failed: ${error && error.message}`;
  console.error("Knowledge graph viewer failed to initialize:", error);
});
