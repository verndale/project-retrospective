  ...
  for (const name of fs.readdirSync(topicsDir)) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const legacy = cited.length === 0
        ? [...l.matchAll(/\[issue #(\d+)\]/gi)].map((match) => ({ number: Number(match[1]), repository: DEFAULT_REPOSITORY }))
        : [];
      ...
      const states = refs.map((ref) => {
        ...
        if (!stateCache.has(cacheKey)) stateCache.set(cacheKey, lookup(String(ref.number), ref.repository, ref.url ? ref : null));
        return stateCache.get(cacheKey);
      }); // consult every cited issue even when the line is already annotated
      if (states.some((state) => state !== "open" && state !== "closed")) continue;
      const labels = refs.map((ref) => ref.url ? formatGithubRef(ref) : `issue #${ref.number}`).join(", ");
