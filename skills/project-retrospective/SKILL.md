...
**7. Continue or hand back** in the checklist shape: components, stable Figma identity/state coverage, review evidence, wiki paths, verification, tracking issue, and exact remaining action. Apply [`references/publication-handoff.md`](references/publication-handoff.md): default `merge` readies, merges, verifies Library `main`, and reconciles the evidence lifecycle without another approval pause; explicit `pull-request` stops at the verified draft PR and explicit `working-tree` stops after local verification. If capability disappears mid-run, persist `Progress` as `code-complete` with `blockedOn: {"code":"figma-capability-lost"}`, report current `figma-pending`, and stop; never invent completion evidence or start another component.
