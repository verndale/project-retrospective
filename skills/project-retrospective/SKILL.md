...
description: Analyzes a completed frontend project and mines it for promotable architecture, reusable components, pipeline rules, and durable team-retrospective knowledge. Inventories and resolves components against ui-design-brain, triages novel labels, drafts catalog/library/orchestration artifacts, captures Confluence retrospectives and accountable actions, and can append retrospective-only backfills to ui-design-evidence. With Action promote or capture, applies validated work to local downstream checkouts; explicitly authorized hands-off runs can continue through commit, pull request, merge, and dependent captures. Use when a project wraps, the user wants a retrospective, pattern harvest, component inventory, catalog gap or alias audit, team-retrospective/post-mortem ingestion, historical evidence backfill, catalog promotion, or server-first multifile library capture.
---
...
17. [`references/publication-handoff.md`](references/publication-handoff.md) — explicit publication authority, automatic commit/PR/merge/dependency continuation, run-specific next steps, and the exact stop/handoff contract. Read for every action.
...
**8. Continue or hand back.** Apply [`references/publication-handoff.md`](references/publication-handoff.md). Rewrite `report.md`'s `## Next steps` from the current artifact counts, exact paths, issue URLs, branch, dependency order, and first unresolved external blocker; rerun the report validator after that edit. Under `Publication: pull-request`, stop at the verified draft PR. Under `Publication: merge`, merge the evidence PR, treat every validator-passing `Verdict: Promote` proposal as approved, apply/merge eligible Brain changes in deterministic path order, reuse exact prior-proposal collisions, then execute every capture that becomes `ready` after its canonical lands. Do not ask for proposal approval again. Otherwise ask the reference's single exact publication question.
...
**5. Continue or hand back** in the checklist shape: edited files (catalog and the wiki paths touched), verification result, tracking issue, and exact deferred captures unlocked by this canonical. Apply [`references/publication-handoff.md`](references/publication-handoff.md): `pull-request` stops at the verified draft PR; `merge` readies, merges, verifies Brain `main`, and immediately continues into each newly ready capture. Otherwise ask its one exact publication question. When the run filed a brain tracking issue (analyze Step 7, [`references/tracking-issues.md`](references/tracking-issues.md)), link it and update the exact capture lifecycle after the promotion lands.
...
| `Publication` | no | explicit current-request authority, otherwise `working-tree` | `working-tree` stops after verified local edits; `pull-request` authorizes commit, push, and verified draft PR; `merge` additionally authorizes ready/merge, closing-keyword issue resolution, and dependent-action continuation. Manual release/tag, Figma publication, protection bypass, and analyzed-project writes remain separate. |
...
- MUST NOT run `git commit`, `push`, open a pull request, or merge without the matching explicit authority resolved by [`references/publication-handoff.md`](references/publication-handoff.md). `Publication: merge` may merge and resolve closing-keyword issues, but MUST NOT force/bypass protection, manually tag/release, directly close unrelated issues, publish Figma, or write to the analyzed project without separate exact authority.
...
- MUST apply [`references/tracking-issues.md`](references/tracking-issues.md) automatically: no approval pause for sanctioned labels, exact issue reuse/creation, issue linking, or required local branches. MUST create no shared branch from artifact presence alone, no library issue before actionable capture preflight, and no issue or branch for `ai-orchestration`. Tracking authority alone is not publication authority; `pull-request` adds commit/push/draft PR, while `merge` also adds ready/merge, closing-keyword issue resolution, and dependent-action continuation. Manual tag/release, Figma publication, protection bypass, and unrelated issue closure remain separate.
