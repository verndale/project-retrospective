...
- `source-parity/<component-key>.json` — exactly one schema-v2 companion per new capture, following [`references/source-parity.md`](references/source-parity.md). Inventory source behavior, visual layout, invariants, and interaction states; record the inspected entry points, tests, styles, build packs, importers, and composed consumers; compare code, Storybook, Figma, and AI resolution; classify every difference and state; record each decision's implementation state; pin and hash every citation; complete decision-phase source-parity review before capture execution. New analyze runs emit v2. Legacy v1 remains readable only for captures already landed/skipped; pending, deferred, ready, or reopened actionable work requires v2. Omit the directory only when there are no captures.
...
Require `schemaVersion: 6`, exact `(canonical, variant)` identity, a validated `sourceParity` decision set, separate non-null `interactionStates`, and a non-null `architecture` plus realization v1 on every executable v2 component. `ready` starts code, `figma-pending` resumes at Figma, `evidence-pending` writes only evidence reconciliation, and `skipped` means code, reviewed unpublished Figma state coverage, post-remediation evidence, and `## Applied` already agree. Missing or inconsistent source coverage/classification, interaction-state disposition, runtime architecture, public API, owned DOM, accessibility behavior/evidence, or consumer responsibility is a hard blocker. The plan returns source parity, interaction states, and architecture separately from `componentJson`; never merge them into it.
...
**2. Apply** one capture at a time, following its validated architecture and the checklist's order: facade/types → tree/parts/hooks → stories → `component.json` → `pnpm exports:sync`. Executing a capture is a rewrite, not a copy: map tokens, remove client coupling, keep server output deterministic, and place `'use client'` only on the planned client leaves (or the deliberate client-mode `index.ts` facade), each no more than 120 physical lines. Paste `componentJson` verbatim and fill `declienting`; do not copy `architecture`, source parity, or interaction states into it. If `interactionStates.status` is `covered`, export `InteractionStates`, render the source-backed visual inventory with narrowly targeted pseudo selectors, and assert target/style differences plus runtime-only behavior in `play`; otherwise retain the explicit not-applicable reason without an empty story. If Action changes the planned API, DOM, keyboard model, accessibility ownership, or state inventory, revise the capture and re-run preflight before continuing.
...
**Side effects (capture)** — adds the server-first component tree, source-backed `InteractionStates` story when applicable, syncs exports, may add a semantic token, and creates an unpublished canonical master plus documentation/state frames in the governed Figma file. It records source-parity decision IDs, stable state node identity, and passed post-remediation source-parity/adversarial/design reviews in `figma/library.json`; the library journal records the findings and fixes. Code Connect is not created or configured. Nothing is committed or published.
...
- MUST NOT create a component directory the library cannot validate: every actionable capture needs schema-v6 preflight with source-parity v2 interaction states, exact structural identity and lifecycle, a runtime architecture and intended realization v1, `index.ts`, a types module, at least two tree/branch/leaf TSX modules, behavior/state-evidence stories, then `component.json` and `pnpm exports:sync`; never leave `components/<slug>/` or `components/<slug>--<variant>/` partially written.
...
- MUST create and review the candidate's unpublished Figma master plus governed state frames (or matching not-applicable registration) before calling a capture complete. Without a write-capable session, report `code complete, Figma promotion blocked`; never invent node or review evidence.
