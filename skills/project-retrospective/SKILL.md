...
description: Analyzes a completed frontend project and mines it for promotable architecture, reusable components, pipeline rules, and durable team-retrospective knowledge. Inventories and resolves components against ui-design-brain, triages novel labels, drafts catalog/library/orchestration artifacts, captures Confluence retrospectives and accountable actions, and can append retrospective-only backfills to ui-design-evidence. Validated runs default to hands-off commit, pull request, merge, catalog promotion, and dependent capture; explicit stop-early modes remain available. Use when a project wraps, the user wants a retrospective, pattern harvest, component inventory, catalog gap or alias audit, team-retrospective/post-mortem ingestion, historical evidence backfill, catalog promotion, or server-first multifile library capture.
---
...
- The user has a validator-approved proposal file, or a hands-off analyze run produced one, and wants it applied to a local ui-design-brain checkout (`Action: promote`).
...
17. [`references/publication-handoff.md`](references/publication-handoff.md) — default hands-off merge, explicit stop-early overrides, automatic dependency continuation, run-specific next steps, and the exact blocker/handoff contract. Read for every action.
18. [`references/execution-ledger.md`](references/execution-ledger.md) — the deterministic JSON event ledger and generated plain-language Markdown audit. Read and update for every action.
...
Create the directory and state the resolved paths before running anything. If any resolved path falls inside `Project`, stop and ask for an `Output` outside it. Resolve an omitted `Publication` to `merge`, initialize `<Output>/execution-log.json` and its generated Markdown twin with `scripts/execution-ledger.cjs`, then record every material outcome through the action per [`references/execution-ledger.md`](references/execution-ledger.md).
...
**5. Self-check.** Record the completed analyze outcomes in the execution ledger, run the validator (see Validation loops), and fix what it reports. Re-rendering the ledger is part of every mutation; never hand-edit `execution-log.md`.
...
**8. Continue or hand back.** Apply [`references/publication-handoff.md`](references/publication-handoff.md), resolving an omitted `Publication` to `merge`. Rewrite `report.md`'s `## Next steps` from the current artifact counts, exact paths, issue URLs, branch, dependency order, and first unresolved external blocker; rerun the report validator after that edit. Under an explicit `Publication: pull-request`, stop at the verified draft PR. In the default `merge` mode, merge the evidence PR, treat every validator-passing `Verdict: Promote` proposal as approved, apply/merge eligible Brain changes in deterministic path order, reuse exact prior-proposal collisions, then execute every capture that becomes `ready` after its canonical lands. Do not ask for proposal or publication approval again. An explicit `working-tree` override stops after verified local edits. Record commit, push, PR, linked issue, checks, merge, downstream action, warning, skip, and blocker outcomes as they occur. The final Evidence reconciliation records `reconcile.complete`, finishes the ledger, re-renders it, and passes `validate-report.cjs` before the last publication.

### Action: ingest-retrospectives

Requires `Data`, `ProjectSlug`, and `Retrospectives`; accepts optional `Date` (today by default). Resolve client identity, canonical CMS key/label, and `priorReports` from the latest existing `<Data>/runs/<ProjectSlug>/` run. Stop if that source metadata uses a legacy or unknown CMS value; this producer does not provide runtime aliases. Stop if the target `<Data>/runs/<ProjectSlug>/<Date>/` already exists. Initialize and maintain its execution ledger per [`references/execution-ledger.md`](references/execution-ledger.md).
...
**4. Wiki.** Author a client-agnostic context-wiki entry in the `Brain` checkout, per [`references/downstream-wiki.md`](references/downstream-wiki.md). Skip with a stated message when `<Brain>/wiki/` is absent. Otherwise read `<Brain>/wiki/MECHANICS.md` and follow it: write `wiki/journal/<date>-<change-slug>.md`, add one `wiki/INDEX.md` Journal line, and — per the proposal type — add a Decisions bullet to `wiki/topics/component-catalog.md`. Ground it in recurrence and the catalog delta (count `N → N+1`), never the client name, run slug, or copy. Then re-run `node scripts/graph/build-graph.cjs` from the `Brain` root so `wiki/connections*` folds in the new entry (exit 0). Do not commit before the complete action is verified.

**5. Continue or hand back** in the checklist shape: edited files (catalog and the wiki paths touched), verification result, tracking issue, and exact deferred captures unlocked by this canonical. Apply [`references/publication-handoff.md`](references/publication-handoff.md): default `merge` readies, merges, verifies Brain `main`, records the verified publication in the execution ledger, and immediately continues into each newly ready capture; explicit `pull-request` stops at the verified draft PR and explicit `working-tree` stops after local verification. When the run filed a brain tracking issue (analyze Step 7, [`references/tracking-issues.md`](references/tracking-issues.md)), link it and update the exact capture lifecycle after the promotion lands.

### Action: capture

Requires `Captures`, `Library`, and `Brain`; optionally accepts `CaptureKeys`, a `Project` checkout override, and `StyleGuide` for the shared Figma guidance file. Applies selected run intents/captures one at a time. Without `Brain`, canonical checking degrades and nothing downstream can compare the directory name against the catalog.

Resolve the run `Output` as the parent of `Captures`. Initialize a missing legacy ledger from sibling `meta.json`, otherwise preserve it, and record all capability, enrichment, implementation, style-guide, Figma, verification, publication, and reconciliation outcomes per [`references/execution-ledger.md`](references/execution-ledger.md).

**1. Gate capability and source before branching.** Follow [`references/library-integrity-checklist.md`](references/library-integrity-checklist.md). Confirm the current write-capable Figma tool, require the exact `library.promotionPattern.componentPage` contract, and run the library's exact `pnpm figma:live:selftest` followed by `pnpm figma:live`. Inspect the configured/supplied `StyleGuide`, read sibling `inventory.json`, and resolve `Project` from it or the override. Prefer its recorded full Git revision. When inventory explicitly reports `sourceSnapshot.strategy: unavailable`, permit only `legacy-untracked` parity that re-hashes every citation and inspected path from the current checkout, rejects every symlink/non-regular file, and records that no Git identity pins those verified bytes. This gate covers fresh and resumed work. Missing writer/page-contract/live/source/style-guide validation stops before any evidence or library branch.
...
Report `orphanedByRun` verbatim. Require schema v6, `source.verified: true`, exact identity, source-parity decisions, separate interaction states/architecture, realization v1, and `figmaPromotion.ready: true` with `writeCapabilityRequired: true`, `status: "ready-for-dev"`, `publicationStatus: "unpublished"`, exact source-parity/adversarial/design passes, the emitted `componentPageContract`, current registry/checklist grammar, `pnpm test:code`, `pnpm figma:live:selftest`, `pnpm figma:live`, `pnpm figma:coverage`, and `pnpm figma:validate`. A `blocked` result stops; exit `6` means promote a deferred sibling proposal first.
...
**5. Promote to Figma, review, and keep standards complete.** Read `figmaPromotion`, the target checklist, and `StyleGuide`. With the confirmed writer, create the unpublished canonical master and documentation from public types, Storybook `argTypes`, and semantic tokens. Promote the complete governed property/variant matrix and every validated interaction state: visual states become connected-instance documentation; runtime-only states retain executable evidence; not-applicable remains explicit. A REST token is read-only validation and does not satisfy this requirement. Before the first write, load `figmaPromotion.componentPageContract` and clone its exact live reference-section appearance and structure. Place the page strictly between its Components boundary pages; preserve its required 528px `01 • Documentation` rail, `02 • Main components`, final numbered Ready for Dev `Interaction states`, separate unnumbered `Publish source`, child names, geometry, gaps, direct-master grid, and descendant containment. Never synthesize page chrome from prose, screenshots, or memory. Then select the component-content precedent: Button for compact component matrices; Section header and Alert for responsive specimens; Tabs for same-page structural alternates. Do not blend their content grammars or replace the executable page shell. Resolve component tokens only through the registry's authoritative code-parity variable collection and stable variable-ID map; never select by display name because legacy collections may contain duplicates. Use 1440/1024/768/390 specimens when applicable; intrinsic components remain intrinsic.

After every Figma write, run `pnpm figma:live:selftest` and the authenticated `pnpm figma:live` before recording a review pass. The live gate must prove Components-group placement, live-reference fills/strokes, exact section/frame structure, direct-master layout, and recursive presentation containment; a screenshot or prose review cannot waive a failure. Then run the source-parity, adversarial, and UI design passes against fresh page renders at reviewable zoom. The UI pass explicitly checks that every child remains inside its card, matrix, frame, and section at each governed width. Fix every code or Figma finding and repeat both machine gates and all affected reviews. When the capture exposes a reusable presentation, variant, state, responsive, token, or accessibility rule absent from `StyleGuide`, add client-agnostic guidance derived from the reviewed current-library precedents and validate the new guidance visually. Do not add a one-off rule or silently choose between contradictory precedents; contradictory standards are a real blocker.

Register the stable node identity in `figma/library.json` with `publicationStatus: "unpublished"`; do not create a Code Connect template or configuration. Record `figma.presentationEvidence` with the inspected precedent plus exact Documentation, Main, Interaction states, and unnumbered Publish source section IDs. Record `figma.tokenBindingAudit` with required code-parity tokens for error, focus, selected/on, and other semantic states; the live validator must reject master aliases outside the authoritative collection. For covered components, add an `Interaction states` documentation frame after Main and every structural alternate, as the final numbered Ready for Dev section, using connected master instances, outside labels, semantic variables, property reuse for `already-represented`, and deterministic frame/instance/component IDs for visual states. Keep runtime-only behavior reasoned and executable rather than static. For not-applicable components, register the reason and `states: []`. Carry the source-parity decision IDs into the registry, then run a fresh post-remediation source-parity pass, an adversarial pass over identity, state differentiation, focus/disabled treatment, properties, aliases, spacing, containment, and breakpoint behavior, and a design pass over hierarchy, alignment, typography, wrapping, intrinsic sizing, and visual consistency. Review cannot pass until Storybook, unpublished Figma state coverage, page structure, and live token bindings agree with source parity and code. Fix every actionable finding in place without deleting/recreating the master and repeat all passes until none remain. If review exposes a code/story/manifest defect, fix that source first, repeat Step 3, then resync Figma. Once intake and every review/validation gate pass, set the governed registration to `status: "ready-for-dev"` by default; publication remains a separate explicit maintainer action. Native Figma Dev Mode readiness is a manual maintainer step when the active API cannot set it.

**6. Wiki and review evidence.** For each component actually written, author a client-agnostic context-wiki entry in `Library` per [`references/downstream-wiki.md`](references/downstream-wiki.md). Skip deferred/blocked/skipped and a checkout with no wiki. Otherwise follow its `wiki/MECHANICS.md`, ground the journal in decision IDs/de-clienting/canonical, record Figma review/fixes/result, point registry review evidence to it, update the index, and run `pnpm graph:build`. Do not commit before the complete action is verified.

Run `pnpm figma:coverage`, `pnpm figma:live:selftest`, `pnpm figma:live`, `pnpm figma:validate`, `pnpm contracts`, `pnpm test`, and `pnpm build` from the Library root. Every command must exit 0 before the component or batch is complete.

Copy the registry proof into modern capture `## Applied`: stable top-level `nodeId`/`nodeKey`, unpublished status, exact `presentationEvidence`, exact `tokenBindingAudit`, exactly the three passed reviews, and exact semantic `stateCoverage`. Visual states require non-empty frame/instance/component IDs, but a child variant component ID need not equal the top-level component-set ID. Runtime-only states have reason and no visual IDs; not-applicable has reason plus empty states.

**7. Continue or hand back** in the checklist shape: components, stable Figma identity/state coverage, style-guide audit/update, review evidence, wiki paths, verification, tracking issue, and exact remaining action. Apply [`references/publication-handoff.md`](references/publication-handoff.md): default `merge` readies, merges, verifies Library `main`, records the verified publication in the execution ledger, and reconciles the evidence lifecycle without another approval pause; explicit `pull-request` stops at the verified draft PR and explicit `working-tree` stops after local verification. If capability disappears mid-run, persist `Progress` as `code-complete` with `blockedOn: {"code":"figma-capability-lost"}`, record and finish the ledger as blocked with the exact resume action, report current `figma-pending`, and stop; never invent completion evidence or start another component.
...
| `Publication` | no | `merge` | `merge` commits, pushes, verifies/readies/merges PRs, resolves closing-keyword issues, and continues dependent actions. `working-tree` stops after verified local edits; `pull-request` stops at a verified draft PR. Manual release/tag, Figma publication, protection bypass, unrelated targets, and analyzed-project writes remain separate. |
| `Proposal` | for promote | — | Path to the validator-approved proposal file to apply. In hands-off `merge` continuation, analyze supplies each Promote proposal automatically. |
...
| `StyleGuide` | capture when configured | library-owned configured guide | Figma design URL for shared client-agnostic UI Design Library guidance. Capture audits it against current component precedents and updates genuinely missing reusable standards. |

**Outputs (analyze)** — all inside `Output`: canonical `execution-log.json`, generated `execution-log.md`, `meta.json`, schema-v1 `inventory.json`, `resolution.json`, `report.md`, `triage.json`, `memory-archive.json`, `proposals/<slug>.md` per Promote candidate, lightweight pending `captures/<slug>.md` intents, and `orchestration-drafts.md`. Analyze emits no `source-parity/`. Optional spec/retrospective inputs add their documented artifacts; `ingest-retrospectives` emits execution log/meta/report plus retrospective artifacts.

**Side effects (analyze, wiki and tracking)** — when `Output` is under a `Data` = ui-design-evidence checkout, create `feat/<project>-<date>-run` from clean aligned `main` before the first write. The run preserves memory/specs, updates the authored wiki, and regenerates committed derived artifacts. Home fallback skips the evidence branch and issue. After validation, automatically reconcile sanctioned labels and file/reuse the private evidence hub plus a client-agnostic brain issue when proposals are pending. Analyze creates no shared-repository branch and no library issue from capture-file presence alone. Default `merge` publication commits, pushes, merges the evidence PR, and continues through validator-approved promotions and newly ready captures; explicit `pull-request` stops at its verified draft PR. See [`references/publication-handoff.md`](references/publication-handoff.md).

**Side effects (promote)** — edits the ui-design-brain working tree, and regenerates that repo's committed graph artifacts as a by-product of verification. When `<Brain>/wiki/` exists, also authors a client-agnostic wiki entry there — `wiki/journal/<date>-<change-slug>.md`, one `wiki/INDEX.md` line, and (per proposal type) a `wiki/topics/component-catalog.md` Decisions bullet — and rebuilds `wiki/connections*` via the brain's own `scripts/graph/build-graph.cjs`. Skipped when that checkout has no `wiki/`. `pull-request` may publish the brain draft PR; `merge` also merges it and continues into unlocked captures.

**Side effects (capture)** — on the evidence run branch, enriches selected intents and adds pinned source-parity v2. On the issue-keyed library branch, adds the server-first component/story/metadata surface and an unpublished governed Figma master/documentation/state coverage. It copies client-neutral Figma identity/review/coverage into `## Applied`. Code Connect is not created or configured. `pull-request` may publish draft PRs in dependency order; `merge` also merges them and verifies lifecycle reconciliation. The Figma library itself remains unpublished.
...
Exit 0 is the pass; `FAIL [check] detail` lines name what to fix. It requires a structured execution ledger with at least one event and byte-exact JSON/Markdown parity for every scope. Pass `--no-brain` when the run had no `Brain`. Pass `--data <Data>` (the ui-design-evidence checkout) so the validator flags a capture or proposal this run drafts that an earlier run already made (`capture-duplicate`/`proposal-duplicate`) — a component already in the library needs no new capture, and a canonical proposed by a prior run should be promoted rather than proposed twice. Warnings do not fail the run but must be read — an exclusion or duplicate warning usually means a candidate should have been dropped.
...
- MUST resolve publication through [`references/publication-handoff.md`](references/publication-handoff.md): retrospective actions default to `merge` only for their deterministic issue/run branches, while explicit `pull-request` and `working-tree` stop earlier. Even in `merge`, MUST NOT force/bypass protection, manually tag/release, directly close unrelated issues, publish Figma, write to unrelated repositories, or write to the analyzed project without separate exact authority.
- MUST maintain the private run execution ledger per [`references/execution-ledger.md`](references/execution-ledger.md): append material outcomes with stable IDs, render Markdown only from canonical JSON, retain failures/warnings/skips, and finish only after the selected boundary or a real blocker is recorded.
...
- MUST apply [`references/tracking-issues.md`](references/tracking-issues.md) automatically: no approval pause for sanctioned labels, exact issue reuse/creation, issue linking, or required local branches. MUST create no shared branch from artifact presence alone, no library issue before actionable capture preflight, and no issue or branch for `ai-orchestration`. Tracking and publication remain separate contracts; the retrospective invocation resolves publication to `merge` by default, while explicit `pull-request` or `working-tree` stops earlier. Manual tag/release, Figma publication, protection bypass, and unrelated issue closure remain separate.
