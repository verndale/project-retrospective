...
Require `schemaVersion: 2` and a non-null `architecture` on every executable component. Missing or inconsistent runtime architecture is a hard blocker. A `skipped` capture is safe only after preflight proves the exact planned modules are non-empty and reachable through the declared client boundary, the root story agrees, and stable `component.json` fields—including provenance and the governed `reuseFingerprint`—match the capture. The plan returns architecture separately from `componentJson`; never merge the two.

**2. Apply** one capture at a time, following its validated architecture and the checklist's order: facade/types → tree/parts/hooks → stories → `component.json` → `pnpm exports:sync`. Executing a capture is a rewrite, not a copy: map tokens, remove client coupling, keep server output deterministic, and place `'use client'` only on the planned client leaves (or the deliberate client-mode `index.ts` facade), each no more than 120 physical lines. Paste `componentJson` verbatim and fill `declienting`; do not copy `architecture` into it.

**3. Verify** each component before starting the next, from the `Library` root: `pnpm contracts`, `pnpm test`, then `pnpm build` (exit 0 required).
...
**5. Stop and hand back** in the shape the checklist specifies: components added, the wiki paths touched, verification result, suggested commits. Do not commit. When the run filed a library tracking issue (analyze Step 7, [`references/tracking-issues.md`](references/tracking-issues.md)), reference it so the operator can check off the applied capture.
...
**Side effects (capture)** — adds a server-first multifile tree under each `components/<slug>/`, deliberately syncs the committed package export map, and may add a semantic token to `src/tokens/semantic.css` when a client token has no semantic home. When `<Library>/wiki/` exists, also authors a client-agnostic wiki entry per written component — `wiki/journal/<date>-add-<slug>-component.md` plus one `wiki/INDEX.md` line — and rebuilds `wiki/connections*` via `pnpm graph:build`. Skipped when that checkout has no `wiki/`. Nothing is committed anywhere.
...
Capture uses the library's own checks, run from the `Library` root: `pnpm contracts` for the fast structural pass, `pnpm test` for typecheck/stories/motion, then `pnpm build` for compiled facade/export parity. Same 3-attempt cap; on exhaustion, revert the component directory and export-map change you were writing and report. `pnpm test` renders every story in a real Chromium — a missing browser (`pnpm exec playwright install chromium`, once per machine) is an environment failure, not a failing component, and does not consume an attempt.
...
- MUST NOT create a component directory the library cannot validate: every capture needs a schema-v2 runtime architecture, `index.ts`, a types module, at least two tree/branch/leaf TSX modules, stories, then `component.json` and `pnpm exports:sync`; never leave `components/<slug>/` partially written.
...
- MUST treat `Project` as read-only. Analyze output goes only inside `Output`, which MUST NOT be inside `Project`; promote edits go only inside the `Brain` working tree; capture edits go only inside the `Library` working tree — under `components/<slug>/`, plus `src/tokens/semantic.css` when a mapping needs a token that does not exist, and the deterministic export-map update produced by `pnpm exports:sync`. A retrospective never leaves artifacts in the repository it analyzed.
