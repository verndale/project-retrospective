...
Capture uses the library's own checks, run from the `Library` root: `pnpm contracts` for the fast structural pass, `pnpm test` for typecheck/Chromium/WebKit/accessibility modes/motion, then `pnpm build` for compiled facade/export parity. Same 3-attempt cap; on exhaustion, revert the component directory and export-map change you were writing and report. A missing browser (`pnpm exec playwright install chromium webkit`, once per machine) is an environment failure, not a failing component, and does not consume an attempt.
