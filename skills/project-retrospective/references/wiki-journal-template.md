...
- Recurrence sources (automatically selected eligible runs): <links or none>
```
...
- `prior_run` is the same-client previous run (or `none`) — distinct from automatically selected cross-project recurrence, which lives only in the Links body.
