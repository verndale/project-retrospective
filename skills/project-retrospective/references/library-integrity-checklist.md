...
3. Before any branch or write, confirm the supported `figma-use` write capability and run the library's exact `pnpm figma:live` command. Inspect the governed component-library Figma file and the supplied or library-configured `StyleGuide` with read access; confirm write access to both when the capture may expose a missing reusable standard. A cover-only or otherwise empty style guide is a documented gap, not evidence that no standards work is needed. Then run `capture-preflight.cjs` with `--figma-writer figma-use --figma-live-validated`, plus optional `--capture-keys` and `--project`. The gate covers fresh and resumed work and validates the current `figma/library.json` and promotion-checklist grammar. Missing capability or required style-guide access stops here; a REST token, read-only tool, or arbitrary label is rejected.
...
6. Audit `StyleGuide` against the completed component and its named precedents. When a reusable rule for properties/variants, interaction states, responsive specimens, semantic tokens, accessibility, or documentation layout is missing, add client-agnostic guidance derived from those current-library designs. Keep guidance minimal and normative; do not copy the component canvas, invent a one-off rule, or treat one capture as a new universal standard. A contradictory precedent is a blocker until the source of truth is resolved. Validate every changed style-guide page structurally and visually.
7. Record `figma.presentationEvidence` with the named precedent plus the exact Documentation, Main, Interaction states, and unnumbered Publish source section IDs. Record `figma.tokenBindingAudit` with the required code-parity token names for each semantic state. Project the private decision into the public client-neutral source-parity contract: preserve the audited family key, set every accepted decision's `implementationKey` to the exact default or compound component directory, and register Figma evidence only on that target implementation. New captures never use a null target. Require the unpublished variant/state frames, complete `stateCoverage` IDs, structural evidence, style-guide audit/update evidence in the execution ledger and library wiki, and a passing live variable audit before the Figma review can pass, then run fresh post-remediation source-parity, adversarial, and UI design passes. The UI design pass checks current-library consistency, hierarchy, alignment, typography, spacing, token bindings, responsive behavior, state differentiation, focus/disabled treatment, containment, and accessibility. Fix findings in place without replacing the master, and repeat until no actionable finding remains.
8. If a finding belongs to React, Storybook, or `component.json`, fix the source, repeat code verification, then resync and re-review Figma. If it identifies a missing shared standard, update `StyleGuide` in the same issue scope and repeat its structural/visual review.
