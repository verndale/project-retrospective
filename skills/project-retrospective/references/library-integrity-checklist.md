...
The stable top-level node may be a component set while state instances connect to child variants; require non-empty IDs and exact semantic state agreement, not `componentNodeId === nodeId`. After all checks pass, copy the same client-neutral Figma registration proof into capture `## Applied`: node ID/key, unpublished status, the three passed reviews, and exact state coverage. Do not copy private citation IDs.
