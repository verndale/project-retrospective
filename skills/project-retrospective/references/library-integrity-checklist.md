...
- Code verification
- Figma promotion, style-guide audit, and design review
...
3. Before any branch or write, confirm the supported `figma-use` write capability. Require `figma/library.json` to expose the exact `library.promotionPattern.componentPage` contract, then run the library's exact `pnpm figma:live:selftest` and authenticated `pnpm figma:live` commands. Inspect the governed component-library Figma file and the supplied or library-configured `StyleGuide` with read access; confirm write access to both when the capture may expose a missing reusable standard. A cover-only or otherwise empty style guide is a documented gap, not evidence that no standards work is needed. Then run `capture-preflight.cjs` with `--figma-writer figma-use --figma-live-validated`, plus optional `--capture-keys` and `--project`. The gate covers fresh and resumed work and validates the current `figma/library.json`, exact component-page contract, live-audit fixture command, and promotion-checklist grammar. Missing capability, page contract, live validation, or required style-guide access stops here; a REST token, read-only tool, arbitrary label, screenshot, or prose claim is rejected.
...
The complete action order is source-parity review → facade/types → tree/parts/hooks → stories → `component.json` → `pnpm exports:sync` → code verification → unpublished Figma promotion with complete variants/states → style-guide audit/update → adversarial/UI-design review → evidence → final verification. Then start the next capture. **One component start-to-complete before the next** — a batch of half-written or code-only captures is the state this order exists to prevent.
...
1. Read `<Library>/figma/PROMOTION-CHECKLIST.md`, inspect `StyleGuide`, inspect and name the live current-library precedent that matches the presentation, and derive the Figma master from the public types, Storybook `argTypes`, semantic tokens, responsive behavior, and validated interaction-state inventory. Resolve tokens through the registry's authoritative code-parity collection and stable variable-ID map, never by a duplicate display name.
2. Create the canonical master and documentation with a write-capable Figma session. Keep it unpublished. Promote every governed public property and variant axis used by the current-library precedent: build the complete valid variant matrix, use component properties for text/boolean/instance-swap behavior, and avoid variant explosion by following the existing family structure. Do not create Code Connect dependencies, scripts, config, templates, credentials, or ungoverned registry fields.
3. Load the preflight's `figmaPromotion.componentPageContract` before writing. Clone its named live reference sections and use its exact Components boundary pages, fills/strokes, required documentation children, coordinates, widths, gaps, master inset, and wrapped publish grid. Never reconstruct that page shell from prose, screenshots, or memory.
4. Place the governed page strictly between the contract's `groupStartPageId` and `groupEndPageId`. Preserve the 528px `01 • Documentation` Ready for Dev rail at x=0; `02 • Main components` to its right; any numbered structural-alternate sections next; and `Interaction states` as the final numbered Ready for Dev section. Keep the canonical master or component set as the only direct child of a separate, unnumbered `Publish source / <Canonical>` section to the right, never embedded in Main or a specimen. Every presentation child must remain inside its card, matrix, frame, and section. Use 1440/1024/768/390 widths when responsive behavior is meaningful; intrinsic components stay capped.
5. Select the component-content reference that matches the presentation: Button for compact component matrices; Section header and Alert for responsive specimens; Tabs for same-page structural alternates. Do not blend those content grammars or alter the executable page shell.
6. Add an `Interaction states` documentation frame after Main and every structural alternate when coverage is `covered`. Use connected instances of the registered master, state labels outside instances, and semantic variables. Use existing properties for `already-represented` states; reproduce pseudo appearances with semantic-variable-bound overrides and documented focus-ring layers. Never detach, rasterize, add a public State property, or alter canonical identity. Record deterministic frame, instance, and component node IDs in `figma.stateCoverage`; runtime-only states keep reason-only registry evidence and executable Storybook proof. A not-applicable registration records its reason and `states: []` without a matrix.
7. Audit `StyleGuide` against the completed component and its named precedents. When a reusable rule for properties/variants, interaction states, responsive specimens, semantic tokens, accessibility, or documentation layout is missing, add client-agnostic guidance derived from those current-library designs. Keep guidance minimal and normative; do not copy the component canvas, invent a one-off rule, or treat one capture as a new universal standard. A contradictory precedent is a blocker until the source of truth is resolved. Validate every changed style-guide page structurally and visually.
8. Record `figma.presentationEvidence` with the named precedent plus the exact Documentation, Main, Interaction states, and unnumbered Publish source section IDs. Record `figma.tokenBindingAudit` with the required code-parity token names for each semantic state. Project the private decision into the public client-neutral source-parity contract: preserve the audited family key, set every accepted decision's `implementationKey` to the exact default or compound component directory, and register Figma evidence only on that target implementation. New captures never use a null target. Require the unpublished variant/state frames, complete `stateCoverage` IDs, structural evidence, style-guide audit/update evidence in the execution ledger and library wiki, and a passing live variable audit before the Figma review can pass. After every write, run `pnpm figma:live:selftest` and authenticated `pnpm figma:live`; no review can pass while either fails. Then inspect fresh page renders and run post-remediation source-parity, adversarial, and UI design passes. The UI design pass checks current-library consistency, hierarchy, alignment, typography, spacing, token bindings, responsive behavior, state differentiation, focus/disabled treatment, and recursive containment through the presentation shell. Fix findings in place without replacing the master, and repeat machine gates plus affected reviews until no actionable finding remains.
9. If a finding belongs to React, Storybook, or `component.json`, fix the source, repeat code verification, then resync and re-review Figma. If it identifies a missing shared standard, update `StyleGuide` in the same issue scope and repeat its structural/visual review.

The stable top-level node may be a component set while state instances connect to child variants; require non-empty IDs and exact semantic state agreement, not `componentNodeId === nodeId`. After intake, source parity, adversarial review, UI design review, style-guide audit, and live validation pass, set the governed registration to `status: "ready-for-dev"` by default while keeping publication unpublished and separately controlled. Then copy the same client-neutral Figma registration proof into capture `## Applied`: node ID/key, ready-for-dev status, unpublished status, presentation evidence, token-binding audit, the three passed reviews, and exact variant/state coverage. Record the style-guide audit/update separately in the execution ledger and client-agnostic library wiki entry; do not widen the governed registry payload or copy private citation IDs. Native Figma Dev Mode readiness remains a manual maintainer step when the active API cannot set it.
...
After all Figma reviews and the style-guide audit pass, author the client-agnostic context-wiki entry in the same delivery, per `references/downstream-wiki.md` — the library's own `wiki/MECHANICS.md` owns the format. Skip with a stated message when `<Library>/wiki/` is absent, and write **only for a component actually written**: a `deferred`, `blocked`, or `skipped` capture wrote nothing, so it gets no entry.

- **Journal** — one `wiki/journal/<date>-add-<slug>-component.md` per written component, `topics: []`, `plan: none`, `pr: pending`. Ground *What changed* in the de-clienting and what was kept verbatim, then record the Figma node identity, all three review passes, the style-guide audit/update node IDs, findings, fixes, and final result.
...
Figma: <canonical> → node <node-id>, unpublished; variants/properties → complete; states → complete/not-applicable; source-parity review → passed; adversarial review → passed; UI design review → passed
Style guide: <url>; audit → passed; updated standards → <node ids or none>
Figma evidence: wiki/journal/<date>-add-<slug>-component.md
Figma gates: pnpm figma:coverage → exit 0; pnpm figma:live:selftest → exit 0; pnpm figma:live → exit 0; pnpm figma:validate → exit 0
