...
3. `capture-preflight.cjs` exits 0 **and its plan carries a non-null `manifest`**. Every capture is `ready` or `skipped`; a `blocked` one names its reason and is not yours to work around. Without `Brain` the script degrades — it warns `manifest-absent`, skips the catalog check, and still exits 0 with every capture `ready`. That is a run that verified nothing about the canonicals, and the library's own contracts will not catch it: they compare `component.json` against its own directory name, never against the catalog.
