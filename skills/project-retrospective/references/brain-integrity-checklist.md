...
The ordered procedure for `Action: promote`. Applies one approved proposal to a **local ui-design-brain working tree**, verifies it, then follows the action's publication authority.
...
Next action: <human review/merge of the PR, or exact dependent capture>
```
