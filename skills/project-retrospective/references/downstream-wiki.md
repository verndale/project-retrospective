...
- MUST be append-only — one journal file per change, never overwritten. This wiki step does not publish a partial state; `publication-handoff.md` may authorize commit, push, and a draft PR only for the complete verified action. Merge, tag, release, and issue closure remain prohibited.
